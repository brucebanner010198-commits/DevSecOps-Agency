...
- **Team:** 6 peers: `eval-designer`, `benchmark-runner`, `regression-detector`, `budget-monitor`, `token-compactor`, `skill-evaluator`
- **Model tier:** `sonnet`
- **Purpose:** Use this agent when Evaluation council needs cost attribution work — 4-column × 3-dim token tracking, weekly FinOps report generation, anomaly triage (span > $1, trace > $10, runaway thresholds), prompt-cache savings accounting, quarterl...
