...
- **Team:** 7 peers: `threat-modeler`, `code-auditor`, `compliance-officer`, `dlp-scanner`, `mcp-defender`, `sbom-slsa`, `secrets-vault`
- **Model tier:** `haiku`
- **Purpose:** Use this agent when the CISO (security-lead) needs a hands-on security probe of the actually-implemented code — not the spec.
