...
Status: `v0.5.4` · MIT + imports (see `LICENSES/`). Security policy: [`SECURITY.md`](SECURITY.md). Trust commitments: [`TRUST.md`](TRUST.md). Live ops console: [`command-center/`](command-center/) — redesigned in v0.5.4 using `skills/ui-ux-pro-max` (Dark-OLED tokens · Inter typography · keyboard shortcuts · commit-velocity sparkline · grouped blocking-vs-informing councils) and live at [`https://brucebanner010198-commits.github.io/DevSecOps-Agency/`](https://brucebanner010198-commits.github.io/DevSecOps-Agency/). Build-order priority: **Security & Privacy → Design → Operations → Timely Delivery** (see [`VALUES.md`](VALUES.md) §12).
...
/plugin install devsecops-agency-0.5.4.plugin
