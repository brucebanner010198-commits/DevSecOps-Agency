...
- **[`command-center/`](command-center/)** — **public operations console (v0.2, v0.5.4).** Single `index.html` — no build step — redesigned in v0.5.4 using the Dark-OLED design system generated by `skills/ui-ux-pro-max` (CSS-variable tokens, Inter + JetBrains Mono via Google Fonts, SVG-only icons, `prefers-reduced-motion` guard, `:focus-visible` keyboard rings, 150–250ms transitions). Shows CEO + 16 councils grouped into `Blocking vetoes` (4 tiles, red accent) and `Informing councils` (12 tiles, sky accent), live vital signs fetched from the GitHub API (councils / skills / agents / hooks), today's rhythm of four heartbeats, twelve Trust commitments panel (shows "pending" until 2026-07-22 scorecard), recent commits feed with a 7-day commit-velocity sparkline, Schedule-A founding-documents grid, five one-click meeting launchers (Call a CEO meeting · Convene a council · Issue a new directive · Schedule a drill · Publish Trust Scorecard), and keyboard shortcuts (R refresh · C CEO meeting · D directive · ? shortcuts overlay · Esc close dialog). Each launcher copies a pre-written slash-command prompt for Claude. Auto-deploys to GitHub Pages via `.github/workflows/pages.yml`; portable to Cloudflare Pages, Vercel, Netlify, or a Cowork artifact. Maintenance procedure in [`skills/command-center-web`](skills/command-center-web/SKILL.md).
- **[`COST-AWARENESS.md`](COST-AWARENESS.md)** — **twelve cost-discipline commitments (v0.5.5).** Net-new root document modeled on `TRUST.md`. Each commitment carries Claim / How-to-verify / If-we-miss. CSRE owns; CAO countersigns. Quarterly cost scorecard (first publish 2026-07-22) sits alongside the trust scorecard. Non-waivable: pre-deploy cost estimate at Phase 6 (§2.4), and any cost-driven downgrade of Security or Design routes through Article X as USER-ONLY (§2.12).
- **Cloud capability skills (v0.5.5, imported from `google/skills` under Apache-2.0; provenance: [`LICENSES/APACHE-2.0-google-skills.txt`](LICENSES/APACHE-2.0-google-skills.txt)):** [`skills/cloud-run-basics`](skills/cloud-run-basics/SKILL.md) (serverless container runtime — Services, Jobs, Worker pools — with the listen-on-0.0.0.0+$PORT golden rule), [`skills/gke-basics`](skills/gke-basics/SKILL.md) (Kubernetes on GKE with a 21-file reference library covering golden-path Autopilot, Workload Identity, Gateway API, autoscaling, compute classes, observability, multi-tenancy, batch+HPC, GPU/TPU inference, backup+DR, storage, reliability, upgrades), [`skills/gcp-auth`](skills/gcp-auth/SKILL.md) (the universal Google Cloud auth recipe — ADC, service accounts, impersonation, Workload Identity Federation, OIDC tokens, never-download-service-account-keys), [`skills/waf-cost-optimization`](skills/waf-cost-optimization/SKILL.md) (Google WAF Cost Optimization pillar with 4 principles + FinOps products + 10-item checklist), [`skills/networking-observability`](skills/networking-observability/SKILL.md) (SRE log+metric investigation with explicit anti-theater boundary rules — `no discrepancy loops`, `conclusive acceptance of inactivity`, `ban on auxiliary scripting` — that mirror Constitution §8). Council ownership: CTO+VPO own cloud-run-basics; CTO owns gke-basics; CISO+CTO jointly own gcp-auth; CSRE owns waf-cost-optimization and networking-observability.
...
/plugin install devsecops-agency-0.6.2.plugin
