...
- **0.3.6** — *Audited MIT imports from `qwibitai/nanoclaw` (MIT, © 2026 Gavriel).* Two new skills + one reference doc + one repo-root license notice. Zero new specialists, zero new injection surface, zero TypeScript source ported. `skills/messaging-formatting/` is a **dialect router** — before any `send_message` / `slack post` / `teams post` / `discord webhook` call, detect the target surface (Slack mrkdwn vs. Discord Markdown vs. Microsoft Teams vs. email vs. CommonMark) and format for it. Construct-by-construct delta tables (Slack `*bold*` vs. Discord `**bold**` vs. CommonMark `**bold**`; Slack `<url|label>` vs. Discord `[label](url)`; Slack numbered lists fail, Discord's work; Slack has no heading primitive, Discord and Teams do; Teams needs `<at>Display Name</at>` + a payload array; email can't use emoji shortcodes), length limits per surface (Slack 40,000 / Discord 2,000 / Teams 25 KB / email subject 78 chars), and four-step detection (explicit param → JID/URL shape → folder prefix → ask). Replaces the "pasted CommonMark into Slack and it printed literally" failure mode for every specialist that writes to a human chat client — `status-writer`, `incident-comms`, `standup`, `stakeholder-update`, `daily-briefing`, `release-announcement`, `notify`, `escalate`. `skills/container-isolation-posture/` is a **design-guidance skill** (no executable code) that codifies a nine-rule sandbox posture as a reviewable checklist: (1) container/VM is primary boundary not app-level checks, (2) mount allowlist stored outside the sandbox and unreachable from agents, (3) fail-closed config loaders (missing = block, never permit-by-default), (4) block-by-default sensitive-path patterns (`.ssh .gnupg .aws .azure .gcloud .kube .docker credentials .env .netrc .npmrc id_rsa id_ed25519 private_key .secret`), (5) `realpath()` symlink resolution before every policy check, (6) container-path validation rejecting absolute paths, `..` traversal, `:` option-injection, and empty strings, (7) read-only project root with writable paths listed explicitly, (8) per-tenant session + scratch + memory isolation, (9) credentials routed through a host-side vault and never entering the container env / stdin / files / `/proc`. Ten-point PR review checklist pastes verbatim into any PR that adds or changes an agent-execution surface; pairs with `sandbox` (execution), `mcp-defense` (consumer-side hardening), `mcp-author`/`mcp-authoring` (producer-side), `injection-defense`, `secrets-vault`, `red-team`, `sdlc-patterns`. `skills/webapp-testing/references/agent-browser-patterns.md` is a **reference doc** (not a standalone skill) distilled from nanoclaw's `agent-browser` container skill — reconnaissance-then-action workflow, five semantic-locator axes (role + name, visible text, form label, placeholder, test id), wait primitives (never sleep unless no meaningful signal exists), authenticated-state pattern with secret-handling gotchas (saved `auth.json` is a credential, not a fixture), six anti-patterns (clicking without a snapshot, stale refs after navigation, shell-injection via `--server` args, no network isolation, running under a real user account in production, time-based waits), all mapped to Playwright and Claude-in-Chrome MCP equivalents. The `agent-browser` binary itself is **NOT a dependency** — pattern reference only. Non-imports (deliberate): `capabilities` and `status` container skills (hardcoded to nanoclaw's `/workspace/project` + `mcp__nanoclaw__*` namespace — not portable); `docs/skills-as-branches.md` (describes a different project's distribution model — git branches as skills, merge-forward CI with Claude-resolved conflicts, community marketplaces via `extraKnownMarketplaces` — interesting but the agency ships as signed `.plugin` archive with GitHub Releases); all TypeScript source under `src/` (container-runner, IPC, router, scheduler, channels, db, sessions, mount-security, sender-allowlist — host-daemon infrastructure with no plugin runtime to host it); `Dockerfile`, `agent-runner`, `launchd/*.plist`, `config-examples/`, `scripts/install.sh`, `repo-tokens/`, `assets/` (8.5 MB). Audit findings called out explicitly: upstream's `sender-allowlist.ts` defaults to fail-OPEN (`allow: '*', mode: 'trigger'`) on missing config — flagged as tradeoff in `container-isolation-posture` rule 3; `container/Dockerfile` uses unpinned `npm install -g agent-browser @anthropic-ai/claude-code` — flagged in rule 9; neither pattern propagates into the plugin. License handling: MIT requires preserving the copyright notice + permission text in "all copies or substantial portions" — satisfied via `LICENSES/MIT-nanoclaw.txt` at repo root (full MIT text + per-file provenance), per-skill `LICENSE.txt` markers in each new skill directory, and explicit `lineage:` frontmatter in both new SKILL.md files naming the upstream source file.
