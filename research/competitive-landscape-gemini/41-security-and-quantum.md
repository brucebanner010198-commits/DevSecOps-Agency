...
- **Transport is already shipping hybrid:** **X25519MLKEM768** is default in Chrome (since Chrome 131, Nov 2024; non-disableable by Chrome 138) and adopted by Cloudflare/Apple; Cloudflare reported that **57.4% of browser connections** use X25519MLKEM768 hybrid key share in early 2026 (milestone of >50% human-initiated web traffic PQC-protected crossed late Oct 2025). [7][8]
...
### The nuance that makes this a wedge (The PQC Reframe)
- **Zero AI-OS / AI-workforce rivals (Category 2) mention PQC** in their product/trust messaging (Warmwind states only "end-to-end encryption" — doc 10; Flowith/Rabbit/Genspark/Manus/OpenAI/Anthropic/Amazon: none found — docs 11, 15, 16; enterprise workforce + startups: none found — docs 21, 22).
...
- **CRITICAL REFRAME:** Post-Quantum Cryptography is **first-mover hygiene, not a durable technical moat** (a rival can match it in a single development sprint). However, the paranoid user demands it due to **"Harvest Now, Decrypt Later" (HNDL)** threats targeting their entire digital life. When delivered via user-owned cryptographic custody (keys held on local hardware enclaves), it represents an architectural alignment that platform players cannot match without abandoning their cloud-data business model.
...
- (b) **VALIDATED with nuance** — Category-1 OSes are migrating; **our category is silent**; PQC is first-mover hygiene deliverable above a reused kernel, serving as a key trust differentiator when paired with sovereign local keys.
...
8. Cloudflare — PQC traffic share (57.4% browser connections in early 2026) — https://radar.cloudflare.com/post-quantum — accessed 2026-06.
