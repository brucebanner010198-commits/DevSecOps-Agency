...
## Audited MIT imports — messaging-formatting + container-isolation-posture + agent-browser patterns (v0.3.6)

- **Source.** All three additions derive from `qwibitai/nanoclaw@v1.2.53` (MIT, © 2026 Gavriel). nanoclaw is an *application* (multi-channel personal-Claude runtime with its own container runtime, IPC, scheduler, and channel registry) — **not** a skill library. The bulk of the repo (`src/`, `setup/`, `launchd/`, `container/Dockerfile`, `config-examples/`, `groups/`, `assets/`, `scripts/`) is host-daemon infrastructure and is deliberately NOT portable. Only three pieces earned import.
- **`skills/messaging-formatting/`** (agency-authored, SKILL.md derived from nanoclaw's `container/skills/slack-formatting/SKILL.md`). Dialect router for Slack mrkdwn vs. Discord vs. Microsoft Teams vs. email vs. CommonMark. Four-step detection (explicit param → channel/JID/URL shape → folder-name fallback → ask), construct-by-construct delta tables for each surface, length limits (Slack 40,000 / Discord 2,000 / Teams 25 KB / email subject 78 chars), worked examples showing the same status update three ways, eight ranked anti-patterns. Extended beyond upstream's Slack-only scope to cover four surfaces. Replaces the "pasted CommonMark into Slack and it rendered literally" failure mode for any specialist that posts to a human channel — status-writer, incident-comms, standup, stakeholder-update, daily-briefing, release-announcement, notify, escalate.
- **`skills/container-isolation-posture/`** (agency-authored, design guidance distilled from nanoclaw's `docs/SECURITY.md` + `src/mount-security.ts`). **Zero TypeScript source ported.** Codifies a nine-rule sandbox posture as a reusable checklist: (1) container/VM is primary boundary not app-level checks, (2) mount allowlist lives outside the sandbox and cannot be mutated from inside, (3) fail-closed config loaders (missing = block), (4) block-by-default sensitive-path patterns (.ssh .gnupg .aws .azure .gcloud .kube .docker credentials .env .netrc .npmrc id_rsa id_ed25519 private_key .secret), (5) realpath() before every policy check, (6) container-path validation rejecting absolute / `..` / `:` / empty, (7) read-only project root with writable paths listed explicitly, (8) per-tenant session + scratch + memory isolation, (9) credentials never enter the sandbox (routed through a host-side vault). Ten-point PR review checklist paste-able into any new agent-execution surface. Pairs with `sandbox` (execution skill), `mcp-defense` (consumer-side hardening), `mcp-author`/`mcp-authoring` (producer-side), `injection-defense`, `secrets-vault`, `red-team`, `sdlc-patterns`.
- **`skills/webapp-testing/references/agent-browser-patterns.md`** (new reference doc inside the existing `webapp-testing` skill). Pattern reference distilled from nanoclaw's `container/skills/agent-browser/SKILL.md`. Reconnaissance-then-action workflow, five semantic-locator axes (role + name, visible text, form label, placeholder, test id), wait primitives (never sleep unless no meaningful signal), authenticated-state pattern with secret-handling gotchas, data-extraction pattern, six anti-patterns, mapping to Playwright and Claude-in-Chrome MCP. The `agent-browser` binary itself is **NOT** a plugin dependency — pattern reference only, applicable to any semantic-locator browser driver.
- **Injection-surface accounting:** v0.3.6 adds **zero** new injection surface. No SKILL.md carries shell-exec, no TypeScript source was ported, `messaging-formatting` and `container-isolation-posture` are pure text with no script payloads, `agent-browser-patterns.md` is reference material. The two minor findings in upstream — `sender-allowlist.ts` fail-OPEN default on missing config, and `container/Dockerfile` unpinned `npm install -g` — are called out explicitly as tradeoff notes in `container-isolation-posture` rules 3 and 9 and do not propagate into the plugin (no such code was imported).
- **IP-lineage (`skills/ip-lineage`):** v0.3.6 carries two skills with MIT-nanoclaw lineage plus one reference doc with the same lineage. MIT §'s copyright-notice and permission-text preservation satisfied at `LICENSES/MIT-nanoclaw.txt` at repo root plus per-skill `LICENSE.txt` markers in each new skill directory. `messaging-formatting/SKILL.md` and `container-isolation-posture/SKILL.md` carry an explicit `lineage:` frontmatter field.
- **No new specialists.** v0.3.6 wires into existing councils: `messaging-formatting` is invoked by comms/status/notify/standup/incident-comms specialists; `container-isolation-posture` is a reviewer checklist invoked by `mcp-defender`, `mcp-author`, `sandbox-runner`, `injection-defense`, `red-team` specialists; `agent-browser-patterns.md` is internal reference consumed by `webapp-testing` users (`e2e-tester`, `qa-lead`).
- **Non-imports (deliberate):**
  - **`capabilities` and `status` container skills** — hardcoded to nanoclaw's `/workspace/project`, `/workspace/group`, and `mcp__nanoclaw__*` tool namespace. Not portable.
  - **`docs/skills-as-branches.md`** — interesting distribution model (skills as git branches, merge-forward CI with Claude-resolved conflicts, community marketplaces) but describes a different project's distribution system, not a skill. The agency ships as a signed `.plugin` archive with GitHub Releases + tags.
  - **All TypeScript source** (`src/container-runner.ts`, `container-runtime.ts`, `db.ts`, `group-folder.ts`, `group-queue.ts`, `ipc.ts`, `ipc-auth.test.ts`, `mount-security.ts`, `remote-control.ts`, `router.ts`, `routing.test.ts`, `sender-allowlist.ts`, `session-cleanup.ts`, `task-scheduler.ts`, `timezone.ts`, `channels/registry.ts`) — host-process infrastructure for running nanoclaw as a daemon. No runtime in the plugin to host it.
  - **`Dockerfile`, `agent-runner`, `launchd/*.plist`, `config-examples/*`, `scripts/install.sh`, `repo-tokens/*`, `assets/*` (8.5 MB)** — not plugin content.
